
import React, { useState } from 'react';
import { toast } from 'sonner';
import { Color } from '@/services/colorService';

interface AddColorsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (colors: Color[]) => void;
  onAdd: (color: Color) => void;
  onRemove: (index: number) => void;
  currentColors: Color[];
}

const AddColorsModal: React.FC<AddColorsModalProps> = ({ isOpen, onClose, onSave, onAdd, onRemove, currentColors }) => {
  const [colors, setColors] = useState<Color[]>(currentColors);
  const [newColor, setNewColor] = useState<Color>({ 
    colorId: 0,
    name: '', 
    hex: '#000000'
  });

  if (!isOpen) return null;
  
  const handleAddColor = () => {
    if (!newColor.name.trim()) {
      toast.error('Please enter a color name');
      return;
    }
    
    if (colors.some(color => color.name?.toLowerCase?.() === newColor.name?.toLowerCase?.())) {
      toast.error('This color already exists');
      return;
    }
    onAdd(newColor);
    setColors([...colors, { ...newColor }]);
    setNewColor({ colorId: 0, name: '', hex: '#000000' });
  };

  const handleRemoveColor = (index: number) => {
    const updatedColors = [...colors];
    updatedColors.splice(index, 1);
    setColors(updatedColors);
    onRemove(index);
  };
  
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
    setNewColor(prev => ({ ...prev, [name]: value }));
  };

  const handleSave = () => {
    onSave(colors);
    onClose();
  };
  
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6 animate-fade-in">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-medium text-gray-900 dark:text-white">Default Color Options</h3>
          <button 
            onClick={onClose} 
            className="text-gray-400 hover:text-gray-500 focus:outline-none"
            type="button"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <line x1="18" y1="6" x2="6" y2="18"></line>
              <line x1="6" y1="6" x2="18" y2="18"></line>
            </svg>
          </button>
        </div>
        
        <div className="space-y-4">
          <div className="flex items-end gap-2">
            <div className="flex-1">
              <label htmlFor="colorName" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Color Name
              </label>
              <input
                id="colorName"
                type="text"
                value={newColor?.name}
                onChange={(e) => setNewColor({ ...newColor, name: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                placeholder="Enter color name"
              />
            </div>
            <div className="w-24">
              <label htmlFor="colorCode" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Color
              </label>
              <input
                id="colorCode"
                type="color"
                value={newColor?.hex}
                onChange={(e) => setNewColor({ ...newColor, hex: e.target.value })}
                className="w-full h-10 border border-gray-300 dark:border-gray-600 rounded cursor-pointer"
              />
            </div>
            <button
              onClick={handleAddColor}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 h-10"
              type="button"
            >
              Add
            </button>
          </div>
          
          <div className="mt-4 max-h-60 overflow-y-auto">
            <h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-2">Current Colors:</h4>
            {colors.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-gray-400">No colors added yet</p>
            ) : (
              <ul className="space-y-2">
                {colors.map((color, index) => (
                  <li 
                    key={index} 
                    className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-2 rounded-md"
                  >
                    <div className="flex items-center">
                      <div
                        className="w-6 h-6 mr-2 rounded-full border border-gray-300 dark:border-gray-600"
                        style={{ backgroundColor: color?.hex || '#000000' }}
                      ></div>
                      <span className="text-sm text-gray-700 dark:text-gray-300">{color.name}</span>
                    </div>
                    <button
                      onClick={() => handleRemoveColor(index)}
                      className="text-red-500 hover:text-red-700 dark:hover:text-red-400"
                      type="button"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <line x1="18" y1="6" x2="6" y2="18"></line>
                        <line x1="6" y1="6" x2="18" y2="18"></line>
                      </svg>
                    </button>
                  </li>
                ))}
              </ul>
            )}
          </div>
          
          <div className="flex justify-end gap-3 pt-4">
            <button
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
              type="button"
            >
              Cancel
            </button>
            {/* <button
              onClick={handleSave}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700"
              type="button"
            >
              Save Color
            </button> */}
          </div>
        </div>
      </div>
    </div>
  );
};

export default AddColorsModal;
